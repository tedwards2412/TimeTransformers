...
def parameter_MSE_CRPS_scaling_plot():
    ...
    for parameter_count, LR in zip(parameter_count_list, highest_LR):
        # file_name = f"results/parameterscaling_{parameter_count}_studentT_training.json"
        file_name = f"results/maxLRscaling_{parameter_count}_{LR}.json"
        ...
        min_test_loss.append(min(model_dict["test_losses"]) + 2)
